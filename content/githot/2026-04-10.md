...
3. Ingest existing history: `mempalace ingest --source cursor_history` or pipe raw text — assign it to a wing (e.g. 'gradland') and hall (e.g. 'architecture-decisions').
4. Query from your app: hit the MCP endpoint before each LLM call to inject relevant past context — e.g. `GET /memory/search?q=supabase+rls+policy&wing=gradland` returns the top-k verbatim chunks to prepend to your system prompt.
