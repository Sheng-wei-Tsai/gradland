...
function FlashcardsTab({ concepts }: { concepts: Concept[] }) {
  ...
  }, [idx, total]); // deps prevent stale closure + stop re-registering on every render
...
export default function StudySession({
  ...
  const videoId = (serverVideoId ?? params.videoId) as string;

  const [meta,           setMeta]           = useState<VideoMeta | null>(initialMeta);
  ...
  useEffect(() => {
    if (meta) return; // skip if server already provided meta
  ...
  const [prefetchedQuiz, setPrefetchedQuiz] = useState<QuizQuestion[] | null>(initialQuiz);
  const quizPrefetchStarted = useRef(initialQuiz !== null); // don't re-fetch if server provided
  ...
  // Skip entirely when the server wrapper already provided a cached guide.
  const analysisStarted = useRef(initialGuide !== null);
  useEffect(() => {
