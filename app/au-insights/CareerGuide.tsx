            ...
            <tbody>
              {[
                ...
                <tr key={path} style={{ borderBottom: '1px solid var(--parchment)', background: i % 2 === 0 ? 'transparent' : 'rgba(245,240,235,0.4)' }}>
                  ...
                  <td style={{ padding: '0.6rem 0.7rem', color: demand === 'Very High' ? '#10b981' : demand === 'High' ? '#f59e0b' : demand === 'Growing' ? '#7c3aed' : 'var(--text-muted)' }}>{demand}</td>
  ...
  },
  {
    ...
    content: (
      <div>
        ...
        <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '8px', padding: '0.9rem 1rem' }}>
          <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#166534', marginBottom: '0.5rem' }}>How to stand out</div>
          <ul style={{ margin: 0, paddingLeft: '1.2rem' }}>
            {[
              ...
              <li key={tip} style={{ fontSize: '0.82rem', color: '#15803d', marginBottom: '0.3rem', lineHeight: 1.5 }}>{tip}</li>
            ))}
          </ul>
        </div>
      </div>
    ...
    content: (
      <div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '0.8rem', marginBottom: '1.2rem' }}>
          <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '8px', padding: '0.9rem 1rem' }}>
            <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#166534', marginBottom: '0.5rem' }}>✓ AU-Specific Rules</div>
            <ul style={{ margin: 0, paddingLeft: '1.2rem' }}>
              {[
                ...
                <li key={rule} style={{ fontSize: '0.82rem', color: '#15803d', marginBottom: '0.3rem', lineHeight: 1.5 }}>{rule}</li>
              ))}
            </ul>
          </div>
          <div style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: '8px', padding: '0.9rem 1rem' }}>
            <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#991b1b', marginBottom: '0.5rem' }}>✗ Common Mistakes</div>
            <ul style={{ margin: 0, paddingLeft: '1.2rem' }}>
              {[
                ...
                <li key={rule} style={{ fontSize: '0.82rem', color: '#dc2626', marginBottom: '0.3rem', lineHeight: 1.5 }}>{rule}</li>
              ))}
            </ul>
          </div>
        </div>
        <div style={{ background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: '8px', padding: '0.9rem 1rem' }}>
          <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#92400e', marginBottom: '0.5rem' }}>Tailoring for AU Companies</div>
          <ul style={{ margin: 0, paddingLeft: '1.2rem' }}>
            {[
              ...
              <li key={tip} style={{ fontSize: '0.82rem', color: '#92400e', marginBottom: '0.3rem', lineHeight: 1.5 }}>{tip}</li>
            ))}
          </ul>
        </div>
      </div>
      ...
      <div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.8rem', marginBottom: '1.2rem' }}>
          {[
            ...
            { label: 'Melbourne IT workers', value: '~60,000', color: '#7c3aed' },
            { label: 'AU vs US market size', value: '~1/10th', color: '#0369a1' },
            { label: 'Avg. grad base (SWE)', value: '$75k–$100k', color: '#10b981' },
          ].map(stat => (
        ...
        <div style={{ marginBottom: '1rem' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
            {['AWS', 'Azure', 'React / Next.js', 'TypeScript', 'Python', 'Kubernetes', 'Docker', 'Terraform', 'Cybersecurity', 'Data Engineering'].map(skill => (
              <span key={skill} style={{ fontSize: '0.82rem', fontWeight: 600, padding: '0.25rem 0.7rem', borderRadius: '99px', background: '#fef4f0', border: '1px solid #f5c6b4', color: 'var(--terracotta)' }}>{skill}</span>
            ))}
          </div>
        </div>

        <div style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: '8px', padding: '0.9rem 1rem' }}>
          <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#991b1b', marginBottom: '0.5rem' }}>Common misconceptions</div>
          <ul style={{ margin: 0, paddingLeft: '1.2rem' }}>
            {[
              ...
              <li key={m} style={{ fontSize: '0.82rem', color: '#dc2626', marginBottom: '0.3rem', lineHeight: 1.5 }}>{m}</li>
            ))}
          </ul>
        </div>
      </div>
  ...
  },
  {
    ...
    content: (
      <div>
        ...
        <div style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: '8px', padding: '0.9rem 1rem' }}>
          <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#991b1b', marginBottom: '0.5rem' }}>Red flags in offers</div>
          <ul style={{ margin: 0, paddingLeft: '1.2rem' }}>
            {[
              ...
              <li key={flag} style={{ fontSize: '0.82rem', color: '#dc2626', marginBottom: '0.3rem', lineHeight: 1.5 }}>{flag}</li>
            ))}
          </ul>
        </div>
      </div>
  ...
  },
  {
    ...
    content: (
      <div>
          ...
          {[
            ...
            <div key={step.years} style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
              <div style={{
                ...
                background: '#fef4f0', border: '1px solid #f5c6b4',
        ...
        <div style={{ background: '#f0f9ff', border: '1px solid #7dd3fc', borderRadius: '8px', padding: '0.9rem 1rem' }}>
          <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#0c4a6e', marginBottom: '0.5rem' }}>AU-Specific Advice</div>
          <ul style={{ margin: 0, paddingLeft: '1.2rem' }}>
            {[
              ...
              <li key={tip} style={{ fontSize: '0.82rem', color: '#0369a1', marginBottom: '0.3rem', lineHeight: 1.5 }}>{tip}</li>
            ))}
          </ul>
        </div>
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem', paddingBottom: '4rem' }}>
      <div style={{
        ...
        fontSize: '0.85rem', color: '#14532d', lineHeight: 1.6,
      }}>
