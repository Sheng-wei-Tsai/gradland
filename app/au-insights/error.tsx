...
export default function AuInsightsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[AuInsightsError]', error.digest ?? error.message, error);
    fetch('/api/log-error', {
      method: 'POST',
  ...
  return (
    <div style={{ maxWidth: '480px', margin: '0 auto', padding: '6rem 1.5rem 4rem', textAlign: 'center' }}>
      ...
      <p style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', lineHeight: 1.7, marginBottom: '2rem' }}>
        Something went wrong loading the AU Insights data. Please try again.
      </p>
